...
from map.domain.value_objects.route_planner_vo import (
    TransportMode,
    arrival_point,
    transport_hint as region_transport_hint,
)
...
def _destinations(places: list[PetFriendlyPlace], brief: CourseBrief) -> list[PetFriendlyPlace]:
    """동선 후보 = 크기 동반 가능 + 동물병원 제외 → 이동수단별 목적지 도착점 인근으로 큐레이션."""
    pool = [
        p for p in places
    ...
    if len(pool) <= _CANDIDATE_CAP:
        return pool
    center = arrival_point(brief.region, brief.transport, places=pool)
...
class GeminiRoutePlannerAgent(RoutePlannerAgentPort):
    ...
    def _run_gemini(self, brief: CourseBrief, candidates: list[PetFriendlyPlace], trails: list[Trail]) -> tuple[list[int], str]:
        ...
        hint_line = region_transport_hint(brief.region, brief.transport)
        prompt = (
            f"지역: {brief.region}\n여행 일수: {brief.days}일\n"
...
def _unique_place_ids(ids: list[int]) -> list[int]:
    """LLM이 같은 id를 반복할 때 순서만 유지하고 중복을 제거한다."""
    seen: set[int] = set()
    out: list[int] = []
    for i in ids:
        if i in seen:
...
def _dedup_places(places: list[PetFriendlyPlace]) -> list[PetFriendlyPlace]:
    """id·이름 기준 중복 제거(순서 유지) — 에이전트·풀 합칠 때."""
    seen_ids: set[int] = set()
    seen_names: set[str] = set()
    out: list[PetFriendlyPlace] = []
    for p in places:
        name_key = (p.name or "").strip().replace(" ", "")
        if p.id in seen_ids or name_key in seen_names:
            continue
        seen_ids.add(p.id)
...
def _parse_plan(text: str, valid_ids: set[int]) -> tuple[list[int], str]:
    ...
    try:
        ...
        return [], text.strip()[:200]
    ordered = _unique_place_ids(
        [int(i) for i in data.get("ordered_place_ids", []) if int(i) in valid_ids]
    )
...
def _parse_recommend(text: str, valid_ids: set[int]) -> tuple[str, list[int]]:
    ...
    if match:
        try:
            data = json.loads(match.group(0))
            ids = _unique_place_ids(
                [int(i) for i in data.get("recommend_place_ids", []) if int(i) in valid_ids]
            )
        except (json.JSONDecodeError, ValueError, TypeError):
            ids = []
        visible = text[: match.start()] + text[match.end():]
